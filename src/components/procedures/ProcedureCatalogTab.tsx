...
export function ProcedureCatalogTab({ searchTerm, setSearchTerm, onAddProcedure, onOpenApprovalQueue }: ProcedureCatalogTabProps) {
  ...
  const [selectedDigitalization, setSelectedDigitalization] = useState('all');
  ...
  const categories = ["all", "Citoyenneté", "Éducation", "Commerce", "Logement", "Urbanisme", "Transport", "Fiscalité"];
  const statuses = ["all", "Active", "Inactive", "En révision"];
  const digitalizations = ["all", "Oui", "Non", "Partiellement"];
  ...
  const filteredProcedures = procedures.filter(procedure => {
    ...
    const digitalizationMatch = selectedDigitalization === 'all' || procedure.digitalization === selectedDigitalization;
    return searchMatch && categoryMatch && statusMatch && digitalizationMatch;
  });
        ...
        <CardContent className="p-4">
          <Tabs defaultValue="category">
            <TabsList className="grid grid-cols-3 md:grid-cols-3">
              ...
              <TabsTrigger value="digitalization">Numérisation</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="digitalization" className="mt-4">
              <div className="flex flex-wrap gap-2">
                {digitalizations.map(digitalization => (
                  <Button
                    key={digitalization}
                    variant={selectedDigitalization === digitalization ? "default" : "outline"}
                    size="sm"
                    onClick={() => setSelectedDigitalization(digitalization)}
                  >
                    {digitalization === 'all' ? 'Tous' : digitalization}
                  </Button>
                ))}
              </div>
            </TabsContent>
